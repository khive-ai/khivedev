...
Real-time dashboard for observing Claude Code hooks and system activity.
Shows live events, tool usage, and system metrics using Streamlit with
SQLite database integration and WebSocket real-time updates.
"""
...
st.set_page_config(
    page_title="🦁 khive_claude Monitoring",
    page_icon="🔍",
    layout="wide",
...
CONFIG = {
    ...
    "ENABLE_WEBSOCKET": os.environ.get("KHIVE_ENABLE_WEBSOCKET", "true").lower() == "true",
    "WEBSOCKET_PORT": int(os.environ.get("KHIVE_WEBSOCKET_PORT", 8766)),
}
...
class ClaudeCodeObservabilityDashboard:
    ...
    async def load_events_from_db(
        ...
        try:
            ...
            recent_events = await HookEvent.get_recent(limit=CONFIG["MAX_EVENTS_DISPLAY"])
    ...
    def render_header(self):
        ...
        <div class="main-header">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <h1 style="margin: 0; font-size: 2rem;">🦁 khive_claude Monitoring</h1>
        ...
        with col1:
            if st.button("🔄 Refresh Data", type="primary", help="Force refresh all data from database"):
                st.rerun()

        with col2:
            auto_refresh = st.checkbox("🔁 Auto", value=True, help="Automatically refresh data")

        with col3:
            refresh_rates = [1, 2, 5, 10]
            default_index = refresh_rates.index(CONFIG["DEFAULT_REFRESH_RATE"]) if CONFIG["DEFAULT_REFRESH_RATE"] in refresh_rates else 2
            refresh_rate = st.selectbox("📊 Rate", refresh_rates, index=default_index, help="Auto-refresh interval (seconds)")
    ...
    def render_metrics(self, metrics: Dict[str, Any]):
        ...
        activity_level = "High" if metrics['recent_events_5m'] > 10 else "Medium" if metrics['recent_events_5m'] > 3 else "Low"
        activity_color = "#28a745" if activity_level == "High" else "#ffc107" if activity_level == "Medium" else "#6c757d"
        
        # Calculate event rate
        events_per_minute = round(metrics['recent_events_5m'] / 5, 1) if metrics['recent_events_5m'] > 0 else 0
        
        # Get most active hook type
        most_active_hook = max(metrics["hook_types"].items(), key=lambda x: x[1])[0] if metrics["hook_types"] else "None"
        ...
        with col2:
            hourly_rate = metrics['recent_events_1h'] if metrics['recent_events_1h'] > 0 else 0
            st.markdown(f"""
            <div class="metric-card">
        ...
        with col4:
            avg_events_per_session = round(metrics['total_events'] / metrics['unique_sessions'], 1) if metrics['unique_sessions'] > 0 else 0
            st.markdown(f"""
            <div class="metric-card">
        ...
        with col6:
            agent_color = "#dc3545" if metrics["estimated_active_agents"] > 5 else "#28a745" if metrics["estimated_active_agents"] > 0 else "#6c757d"
            st.markdown(f"""
            <div class="metric-card">
    ...
    def render_hook_types_chart(self, metrics: Dict[str, Any]):
        ...
        hook_data = []
        color_map = {
            "pre_command": "#28a745", "post_command": "#20c997",
            "pre_edit": "#17a2b8", "post_edit": "#20c997", 
            "pre_agent_spawn": "#ffc107", "post_agent_spawn": "#fd7e14",
            "prompt_submitted": "#6f42c1", "notification": "#e83e8c",
            "setup_test": "#6c757d", "debug_test": "#495057"
        }
        ...
        for hook_type, count in metrics["hook_types"].items():
            hook_data.append({
                ...
                "Count": count,
                "Percentage": round((count / sum(metrics["hook_types"].values())) * 100, 1),
    ...
    def render_timeline_chart(self, events: List[Dict[str, Any]], time_range: str = "Today"):
        ...
        if time_range == "Today":
            ...
            end_date = today
            filtered_events = [e for e in events if start_date <= e["datetime"].date() <= end_date]
            group_by = "day"
        ...
        if group_by == "hour" and time_range == "Today":
            ...
            x_labels = [datetime.strptime(d, "%Y-%m-%d").strftime("%m/%d") for d in date_range]
            x_data = date_range
            ...
            for event in filtered_events:
                week_start = event["datetime"].date() - timedelta(days=event["datetime"].weekday())
        ...
        for event_type in sorted(all_event_types):
            ...
            if sum(y_values) > 0:
                fig.add_trace(go.Scatter(
                    ...
                    line=dict(width=0.5, color=color_map.get(event_type, "#6c757d")),
                    stackgroup='one',
        ...
        if group_by == "hour" and time_range == "Today":
            ...
            peak_period = max(total_by_period.items(), key=lambda x: x[1]) if total_by_period else (0, 0)
    ...
    def render_events_table(self, events: List[Dict[str, Any]]):
        ...
        with col2:
            display_limit = st.selectbox("Show", [10, 25, 50, 100], index=1, key="event_limit")
        with col3:
            show_details = st.checkbox("🔍 Show Details", value=False, help="Show expanded event information")

        # Get events to display
        display_events = events[-display_limit:] if len(events) > display_limit else events
        ...
        if show_details:
            ...
            for event in display_events:
                ...
                event_icon = {
                    ...
                    "pre_edit": "📝", "post_edit": "💾", 
                    "pre_agent_spawn": "🚀", "post_agent_spawn": "🎯",
                    "prompt_submitted": "💬", "notification": "🔔",
                    "setup_test": "🧪", "debug_test": "🔬",
                ...
                if event_type == "prompt_submitted":
                    ...
                    details = f"🔔 {severity.upper()}: {message[:80]}..." if len(message) > 80 else f"🔔 {severity.upper()}: {message}"
                elif event_type in ["pre_edit", "post_edit"]:
                    if file_paths:
                        # Show file names (not full paths for cleaner display)
                        file_names = [fp.split('/')[-1] for fp in file_paths[:2]]  # Show max 2 files
                    ...
                    details = f"🤖 {task_desc[:60]}..." if len(task_desc) > 60 else task_desc or "Agent task"
                elif event_type in ["pre_command", "post_command"] and command:
                    # Better command display - show more context
                    details = f"💻 {command[:80]}..." if len(command) > 80 else f"💻 {command}"
                elif command:
                    details = f"💻 {command[:80]}..." if len(command) > 80 else f"💻 {command}"
                else:
            ...
            st.dataframe(
                ...
                height=400,
                column_config={
                    "🕐 Time": st.column_config.TextColumn("Time", width="small", help="Event timestamp"),
                    "🎯 Event": st.column_config.TextColumn("Event Type", width="medium", help="Type of hook event"), 
                    "🛠️ Tool": st.column_config.TextColumn("Tool", width="small", help="Claude Code tool used"),
                    "👤 Session": st.column_config.TextColumn("Session", width="small", help="Session identifier"),
                    "📄 Details": st.column_config.TextColumn("Details", width="large", help="Event details and context"),
                },
                hide_index=True
            )
            ...
            if table_data:
                ...
                with col1:
                    ...
                    for row in table_data:
                        event_type = row["🎯 Event"].split(" ", 1)[1] if " " in row["🎯 Event"] else row["🎯 Event"]
                        event_counts[event_type] = event_counts.get(event_type, 0) + 1
                    st.caption(f"📊 Most active: {max(event_counts, key=event_counts.get)} ({max(event_counts.values())} events)")
                with col2:
    ...
    def render_event_card(self, event: Dict[str, Any], index: int):
        ...
        color_map = {
            ...
            "prompt_submitted": "#6f42c1", "notification": "#e83e8c",
            "setup_test": "#6c757d", "debug_test": "#495057",
            "integration_test": "#17a2b8", "test": "#6c757d"
        }
    ...
    def render_sidebar(self, metrics: Dict[str, Any]):
        ...
        if ws_server_running:
            st.sidebar.success(f"🟢 WebSocket Server: ws://localhost:{CONFIG['WEBSOCKET_PORT']}")
            ...
            if st.sidebar.button("📋 Start Server Command"):
                st.sidebar.code(f"cd khive_claude && uv run python -m khive_claude.cli server --port {CONFIG['WEBSOCKET_PORT']}")
        ...
        with st.sidebar.expander("ℹ️ Help & Info"):
            st.markdown("""
            **khive_claude** monitors Claude Code hook events in real-time.
    ...
    def run(self):
        ...
        with st.expander("🎛️ Advanced Filters", expanded=False):
            ...
            with filter_col1:
                # Event type filter
                all_event_types = sorted(list(set([e.get("event_type", "unknown") for e in events])))
                ...
                # Session filter
                all_sessions = list(set([e.get("session_id", "unknown") for e in events if e.get("session_id")]))
            ...
            with filter_col2:
                # Tool filter
                all_tools = sorted(list(set([e.get("tool_name", "unknown") for e in events])))
        ...
        if selected_event_types:
            filtered_events = [e for e in filtered_events if e.get("event_type") in selected_event_types]
            
        if selected_tools:
            filtered_events = [e for e in filtered_events if e.get("tool_name") in selected_tools]
            
        if selected_sessions:
            filtered_events = [e for e in filtered_events if e.get("session_id") in selected_sessions]
        ...
        if events and 'time_range' in locals():
            ...
            filtered_events = [
                ...
                if start_time <= e["datetime"] <= end_time
            ]
        ...
        if len(filtered_events) != len(events):
            st.info(f"🔍 Showing {len(filtered_events)} of {len(events)} events (filtered)")
        ...
        with timeline_col1:
            time_range_options = ["Today", "Last 7 Days", "Last 30 Days", "This Month", "All Time"]
            default_index = time_range_options.index(CONFIG["DEFAULT_TIME_RANGE"]) if CONFIG["DEFAULT_TIME_RANGE"] in time_range_options else 0
        ...
        with footer_col1:
            ...
            <div style="font-size: 0.875rem; color: #6c757d;">
                <strong>khive_claude</strong> v{__version__} | Real-time Claude Code monitoring
            </div>
            """, unsafe_allow_html=True)
        with footer_col2:
            st.markdown("""
        ...
        with footer_col3:
            ...
            <div style="font-size: 0.875rem; color: #6c757d; text-align: right;">
                <a href="https://github.com/khive-ai/khive_claude" target="_blank" style="color: #1e3c72;">
                    🔗 GitHub
                </a> | 
                <a href="https://github.com/khive-ai/khive_claude/issues" target="_blank" style="color: #1e3c72;">
                    🐛 Report Issue
                </a>
            </div>
            """, unsafe_allow_html=True)
